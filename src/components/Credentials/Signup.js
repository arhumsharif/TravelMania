...
export default function Signup() {
    ...
    const register = async () => {
        let response = await addUser(email.current.value, password.current.value, userType);
        if (response == 404)
        {
            setMessage('block')
        }
        ...
        <div className="flex min-h-screen justify-center items-center">
            <div className="w-full shadow-lg max-w-lg">
                <div className="rounded-2xl p-10 bg-white text-center font-semibold text-xs">
                    ...
                    <Link to='/login'>Log in <span className="underline decoration-solid">here</span></Link>
                </div>
            </div>
        </div>
    );
